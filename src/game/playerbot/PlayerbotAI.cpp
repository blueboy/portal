...
void PlayerbotAI::_HandleCommandGM(std::string &text, Player &fromPlayer)
{
    ...
    else if (ExtractCommand("check", text))
    {
        if (ExtractCommand("autobot", text))
        {
            ...
            {
                SendWhisper("Your command has been received, resetting column 'in_use' for table 'playerbot_autobot_names'. World may freeze for a bit.", fromPlayer);
                PlayerbotMgr::AutobotNamesInUseReset();
                SendWhisper("Autobot names 'in_use' column has been reset.", fromPlayer);
            }
            else
            {
...
void PlayerbotAI::_HandleCommandHelp(std::string &text, Player &fromPlayer)
{
    ...
    if (fromPlayer.GetSession()->GetSecurity() > SEC_PLAYER && (bMainHelp || ExtractCommand("gm", text)))
    {
        ...
        if (!bMainHelp)
        {
            ...
            {
                ch.SendSysMessage(_HandleCommandHelpHelper("gm check autobot", "Lists autobot mechanics you can run a check on.").c_str());
                ch.SendSysMessage(_HandleCommandHelpHelper("gm check talent", "Lists talent mechanics you can run a check on.").c_str());

                if (ExtractCommand("autobot", text))
                {
                    ch.SendSysMessage(_HandleCommandHelpHelper("gm check autobot names", "Checks the autobot names, resetting its 'in_use' column in the database. May freeze the server for a little while.").c_str());
